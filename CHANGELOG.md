...
- 全量 **283 条 0 失败**（`npm test`：35 + 98 + 116 + 34）。
- 新增覆盖：插入模式三态与「两个标志都为真 → 按每次」的优先级、切模式时互斥标志被清掉、`autoSendFirst` 的文件往返与旧文件（只有 `autoSend`）的向后兼容、发送批次在 `blank=false` 时**不带**「仅首次」、跨分类移动的语义与四种 no-op、「会话是否为空」取自槽位快照的接缝、两处共用同一控件。
- 变异测试：把「仅首次」退化成「每次」（忽略 `blank`）与切断互斥清理，护栏分别变红 4 处。
