...
- **自定义**：使用本插件固定样式的菜单（未选中文本时「剪切 / 复制 / 删除」置灰）；「粘贴」需要浏览器剪贴板授权（首次可能弹一次授权提示，被拒绝或无响应时提示「请用 Ctrl+V 粘贴」），Firefox 要把 `about:config` 里的 `permissions.default.clipboard-read` 设为 `1` 才不弹授权窗。
